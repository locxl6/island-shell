...
post_process() {
    ...
    handle_kde_material_you_colors &
    # ponytail: this repo may not include the optional VS Code color helper.
    # Quickshell colors are already generated by matugen, so skip cleanly.
    if [[ -x "$SCRIPT_DIR/code/material-code-set-color.sh" ]]; then
...
switch() {
    ...
    if [ -f "$SHELL_CONFIG_FILE" ]; then
        ...
        # ponytail: dark/light UI buttons need to regenerate Quickshell colors
        # even when persistent app/shell wallpaper theming is disabled.
        if [ "$enable_apps_shell" == "false" ] && [ "$II_FORCE_SHELL_THEME" != "1" ]; then
    ...
    # ponytail: end4's optional terminal/app color pipeline is not always present
    # in this fork. Do not fail the shell theme toggle when those files are absent.
    ...
    if [[ -x "$SCRIPT_DIR/applycolor.sh" ]]; then
...
main() {
    ...
    # ponytail: forced single-color mode — always derive from accentColor, never wallpaper
    config_color="$(get_accent_color_from_config)"
    if [[ ! "$config_color" =~ ^#?[A-Fa-f0-9]{6}$ ]]; then
        config_color="#8095b5"   # fallback accent when user hasn't set one
    fi
    color_flag="1"
    color="$config_color"
    ...
    # ponytail: don't clear accent; still record wallpaper path for display
    if [[ -n "$imgpath" && -z "$noswitch_flag" ]]; then
        set_wallpaper_path "$imgpath"
    fi

    # If type_flag is 'auto', detect scheme type from image (after imgpath is set)
    # ponytail: skip image-based scheme detection in forced color mode
    if [[ "$type_flag" == "auto" && "$color_flag" != "1" ]]; then
